...
/// <summary>OpenSSH BCrypt PBKDF implementation.</summary>
/// <remarks>
/// OpenSSH password-protected private keys use a modified version of the BCrypt algorithm -
/// reference comments in
/// https://github.com/openssh/openssh-portable/blob/master/openbsd-compat/bcrypt_pbkdf.c
/// The differences in IV, hash algorithm, and output expansion mean that any common
/// BCrypt API wouldn't work; hence the custom implementation here.
///
/// This code is originally adapted from
/// https://github.com/sshnet/SSH.NET/blob/master/src/Renci.SshNet/Security/Cryptography/Bcrypt.cs
/// which itself was adapted from BCrypt.NET - https://github.com/BcryptNet/bcrypt.net.
/// However the code here is greatly slimmed down compared to those sources. It does not
/// provide the full BCrypt API, only enough to be able to encrypt/decrypt OpenSSH private keys.
/// </remarks>
internal sealed class BCrypt
{
	// Expanded Blowfish key
	private uint[] p = Array.Empty<uint>();
	...
	public void Pbkdf(byte[] password, byte[] salt, int rounds, byte[] output)
	{
		using (var sha512 = SHA512.Create())
		{
			...
			byte[] hashInput = new byte[salt.Length + block_b.Length];
			for (int block = 1; block <= nblocks; block++)
			{
				...
				Array.Copy(block_b, 0, hashInput, salt.Length, block_b.Length);
				byte[] hsalt = sha512.ComputeHash(hashInput);
	...
	private void Hash(byte[] hpass, byte[] hsalt, byte[] output)
	{
		...
		uint[] buf = new uint[OpenBsdIv.Length];
		Array.Copy(OpenBsdIv, 0, buf, 0, OpenBsdIv.Length);
		for (int i = 0; i < 8; i += 2)
		{
			for (int j = 0; j < 64; j++)
			{
	...
	{
		p = new uint[POrig.Length];
		s = new uint[SOrig.Length];
		Array.Copy(POrig, p, POrig.Length);
		Array.Copy(SOrig, s, SOrig.Length);
	}
	...
	private void Key(byte[] keyBytes)
	{
		...
		for (i = 0; i < plen; i++)
			p[i] = p[i] ^ StreamToWord(keyBytes, ref koffp);
	...
	private void EksKey(byte[] saltBytes, byte[] inputBytes)
	{
		...
		for (i = 0; i < plen; i++)
			p[i] = p[i] ^ StreamToWord(inputBytes, ref passwordOffset);
	...
	{
		const int blowfishNumRounds = 16;

		uint block = blockArray[offset] ^ p[0];
		uint r = blockArray[offset + 1];

		unchecked
		{
			for (uint round = 0; round <= blowfishNumRounds - 2;)
			{
				// Feistel substitution on left word
				uint n = s[(block >> 24) & 0xff];
			...
			blockArray[offset] = r ^ p[blowfishNumRounds + 1];
	...
	private static readonly uint[] OpenBsdIv = new uint[]
	{
