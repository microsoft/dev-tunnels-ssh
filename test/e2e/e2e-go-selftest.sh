...
# E2E self-test: Go client <-> Go server over real TCP sockets.
# Tests 5 algorithm combinations + 3 feature modes (large data, multi-channel, PK auth).
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
INTEROP_DIR="$REPO_ROOT/test/interop/go"
...
# Feature mode tests: mode kex pk enc hmac expected_marker
FEATURE_TESTS=(
  "large ecdh-sha2-nistp384 ecdsa-sha2-nistp384 aes256-gcm@openssh.com hmac-sha2-256 LARGE_DATA_OK"
  "multi ecdh-sha2-nistp256 ecdsa-sha2-nistp256 aes256-ctr hmac-sha2-256-etm@openssh.com MULTI_CHANNEL_OK"
  "pkauth ecdh-sha2-nistp521 ecdsa-sha2-nistp521 aes256-gcm@openssh.com hmac-sha2-512-etm@openssh.com PK_AUTH_OK"
)

PASSED=0
FAILED=0
TOTAL=$(( ${#COMBOS[@]} + ${#FEATURE_TESTS[@]} ))
...
for combo in "${COMBOS[@]}"; do
  ...
  "$BINARY" server "$port" "$kex" "$pk" "$enc" "$hmac" \
    >"$SERVER_LOG" 2>&1 &
  server_pid=$!
  PIDS="$server_pid"
  ...
  run_with_timeout "$TIMEOUT" "$BINARY" client "$port" "$kex" "$pk" "$enc" "$hmac" \
    >"$CLIENT_LOG" 2>&1 || true
...
echo "=== Feature Mode Tests ==="
echo ""

for feature_test in "${FEATURE_TESTS[@]}"; do
  read -r mode kex pk enc hmac expected_marker <<< "$feature_test"
  port=$(find_free_port)

  echo "--- Test: $mode ($kex / $enc) ---"

  SERVER_LOG=$(mktemp)
  CLIENT_LOG=$(mktemp)
  PIDS=""

  # Start server in background with mode.
  "$BINARY" server "$port" "$kex" "$pk" "$enc" "$hmac" "$mode" \
    >"$SERVER_LOG" 2>&1 &
  server_pid=$!
  PIDS="$server_pid"
  ...
  # Run client with timeout and mode.
  run_with_timeout "$TIMEOUT" "$BINARY" client "$port" "$kex" "$pk" "$enc" "$hmac" "$mode" \
    >"$CLIENT_LOG" 2>&1 || true

  # Verify expected marker.
  ok=true
  if ! grep -q "$expected_marker" "$CLIENT_LOG" 2>/dev/null; then
    echo "  FAIL: Client missing marker: $expected_marker"
    ok=false
  fi
...
echo "=== Go Self-Test Results: $PASSED/$TOTAL passed ==="
