...
# E2E signals test: verifies standalone signal delivery, exit-status, and
# exit-signal propagation through SSH channels over real TCP.
set -euo pipefail
...
for combo in "${COMBOS[@]}"; do
  ...
  echo "--- Test: signals ($kex / $enc) ---"

  SERVER_LOG=$(mktemp)
  CLIENT_LOG=$(mktemp)
  PIDS=""

  # Start server in signals mode.
  "$BINARY" server "$port" "$kex" "$pk" "$enc" "$hmac" signals \
    >"$SERVER_LOG" 2>&1 &
  server_pid=$!
  PIDS="$server_pid"
  ...
  run_with_timeout "$TIMEOUT" "$BINARY" client "$port" "$kex" "$pk" "$enc" "$hmac" signals \
    >"$CLIENT_LOG" 2>&1 || true

  # Verify client markers.
  ok=true
  for marker in SIGNAL_SENT EXIT_STATUS_OK EXIT_SIGNAL_OK DONE; do
...
echo "=== Go Signals Test Results: $PASSED/$TOTAL passed ==="
