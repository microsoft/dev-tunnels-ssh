...
# E2E extended-data test: verifies SendExtendedData (stderr) send and receive
# through SSH channels over real TCP.
set -euo pipefail
...
for combo in "${COMBOS[@]}"; do
  ...
  echo "--- Test: extended-data ($kex / $enc) ---"

  SERVER_LOG=$(mktemp)
  CLIENT_LOG=$(mktemp)
  PIDS=""

  # Start server in extended-data mode.
  "$BINARY" server "$port" "$kex" "$pk" "$enc" "$hmac" extended-data \
    >"$SERVER_LOG" 2>&1 &
  server_pid=$!
  PIDS="$server_pid"
  ...
  run_with_timeout "$TIMEOUT" "$BINARY" client "$port" "$kex" "$pk" "$enc" "$hmac" extended-data \
    >"$CLIENT_LOG" 2>&1 || true

  # Verify client markers.
  ok=true
  for marker in EXTENDED_DATA_SEND_OK EXTENDED_DATA_RECV_OK DONE; do
...
echo "=== Go Extended Data Test Results: $PASSED/$TOTAL passed ==="
