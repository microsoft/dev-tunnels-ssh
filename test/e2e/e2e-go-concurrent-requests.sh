...
# E2E concurrent-requests test: verifies non-blocking dispatch over real TCP.
# A blocking request handler on channel 1 must not prevent channel 2 data echo.
set -euo pipefail
...
# Algorithm combinations (same 3 as reconnect/portfwd scripts).
COMBOS=(
...
for combo in "${COMBOS[@]}"; do
  ...
  echo "--- Test: concurrent-requests ($kex / $enc) ---"

  SERVER_LOG=$(mktemp)
  CLIENT_LOG=$(mktemp)
  PIDS=""

  # Start server in concurrent-requests mode.
  "$BINARY" server "$port" "$kex" "$pk" "$enc" "$hmac" concurrent-requests \
    >"$SERVER_LOG" 2>&1 &
  server_pid=$!
  PIDS="$server_pid"
  ...
  run_with_timeout "$TIMEOUT" "$BINARY" client "$port" "$kex" "$pk" "$enc" "$hmac" concurrent-requests \
    >"$CLIENT_LOG" 2>&1 || true

  # Verify client markers.
  ok=true
  for marker in CONCURRENT_REQUEST_OK DONE; do
...
echo "=== Go Concurrent Requests Test Results: $PASSED/$TOTAL passed ==="
