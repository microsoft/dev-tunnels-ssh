...
# E2E pipe-request test: verifies Channel.Pipe request forwarding over real TCP.
# Client connects to server A, which pipes to an internal server B via PipeSession.
# Channel requests and data flow through the pipe bidirectionally.
set -euo pipefail
...
# Algorithm combination (at least 1 required by acceptance criteria).
COMBOS=(
...
for combo in "${COMBOS[@]}"; do
  ...
  echo "--- Test: pipe-request ($kex / $enc) ---"

  SERVER_LOG=$(mktemp)
  CLIENT_LOG=$(mktemp)
  PIDS=""

  # Start server in pipe-request mode.
  "$BINARY" server "$port" "$kex" "$pk" "$enc" "$hmac" pipe-request \
    >"$SERVER_LOG" 2>&1 &
  server_pid=$!
  PIDS="$server_pid"
  ...
  run_with_timeout "$TIMEOUT" "$BINARY" client "$port" "$kex" "$pk" "$enc" "$hmac" pipe-request \
    >"$CLIENT_LOG" 2>&1 || true

  # Verify client markers.
  ok=true
  for marker in PIPE_REQUEST_FORWARDED ECHO_OK DONE; do
...
echo "=== Go Pipe Request Test Results: $PASSED/$TOTAL passed ==="
