...
# E2E rekey test: verifies that key rotation (rekey) mid-session preserves data
# integrity and session health. Uses a low KeyRotationThreshold (4 KB) so that
# sending 16 KB triggers multiple rekeys.
set -euo pipefail
...
# Algorithm combinations (need real encryption for rekey to be meaningful).
COMBOS=(
...
for combo in "${COMBOS[@]}"; do
  ...
  echo "--- Test: rekey ($kex / $enc) ---"

  SERVER_LOG=$(mktemp)
  CLIENT_LOG=$(mktemp)
  PIDS=""

  # Start server in rekey mode.
  "$BINARY" server "$port" "$kex" "$pk" "$enc" "$hmac" rekey \
    >"$SERVER_LOG" 2>&1 &
  server_pid=$!
  PIDS="$server_pid"
  ...
  run_with_timeout "$TIMEOUT" "$BINARY" client "$port" "$kex" "$pk" "$enc" "$hmac" rekey \
    >"$CLIENT_LOG" 2>&1 || true

  # Verify client markers.
  ok=true
  for marker in REKEY_DATA_OK POST_REKEY_ECHO_OK DONE; do
...
echo "=== Go Rekey Test Results: $PASSED/$TOTAL passed ==="
