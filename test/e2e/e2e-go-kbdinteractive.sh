...
# E2E keyboard-interactive auth test: verifies the multi-round-trip
# keyboard-interactive authentication flow (server sends prompt, client
# responds, server validates) over real TCP.
set -euo pipefail
...
for combo in "${COMBOS[@]}"; do
  ...
  echo "--- Test: kbdinteractive ($kex / $enc) ---"

  SERVER_LOG=$(mktemp)
  CLIENT_LOG=$(mktemp)
  PIDS=""

  # Start server in kbdinteractive mode.
  "$BINARY" server "$port" "$kex" "$pk" "$enc" "$hmac" kbdinteractive \
    >"$SERVER_LOG" 2>&1 &
  server_pid=$!
  PIDS="$server_pid"
  ...
  run_with_timeout "$TIMEOUT" "$BINARY" client "$port" "$kex" "$pk" "$enc" "$hmac" kbdinteractive \
    >"$CLIENT_LOG" 2>&1 || true

  # Verify client markers.
  ok=true
  for marker in KBD_AUTH_OK ECHO_OK DONE; do
  ...
  # Also verify server-side verification marker.
  if ! grep -q "KBD_AUTH_VERIFIED" "$SERVER_LOG" 2>/dev/null; then
    echo "  FAIL: Server missing marker: KBD_AUTH_VERIFIED"
    ok=false
  fi
...
echo "=== Go Keyboard-Interactive Auth Test Results: $PASSED/$TOTAL passed ==="
